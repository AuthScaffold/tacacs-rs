...
- **Decompose libraries by file responsibility** — Do not build Rust libraries as massive singular `lib.rs` files. Treat each source file as a reviewable unit with one clear reason to exist and a name that tells reviewers what they will find before opening it. Keep `lib.rs` as the crate facade: crate docs, top-level feature gates, `mod` declarations, intentional `pub use` exports, and only tiny glue that genuinely defines the public surface. Move implementation into focused modules such as `config`, `authorization`, `ffi`, `runtime`, `logging`, `unix`, `transport`, or `test_support` based on the domain boundaries already present in the crate.
- **Split by concept, not by convenience** — Prefer module boundaries that match protocol concepts, platform boundaries, I/O boundaries, ownership/lifetime boundaries, or externally visible API areas. Avoid vague dumping grounds like `utils`, `helpers`, `common`, or `misc` unless the crate already has a well-defined local convention for them. If a helper belongs only to one concept, keep it private inside that concept's module.
- **Optimize for human review** — A reviewer should be able to scan the file tree and predict the contents and risk profile of each file. When a file starts mixing configuration parsing, FFI entry points, environment inspection, IPC calls, logging, and tests, split it before adding more behavior. Prefer several cohesive files over one file that requires reviewers to page through unrelated concerns.
- **Keep module APIs narrow** — Use `pub(crate)` for cross-module seams inside a crate and reserve `pub` exports for the intentional crate API. Re-export public types from `lib.rs` when that improves ergonomics, but keep construction details, parsing helpers, platform shims, and test fixtures private to their modules whenever possible.
